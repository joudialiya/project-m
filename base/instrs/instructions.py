...
class Instruction:
    ...
    def add(self, op):
        self.ops.append(op)
        return self
    def addSubInstruction(self, instruction):
    ...
    def getCycles(self):
        ...
        for op in self.ops:
            op: MicroOp
            # print(op.__class__, isCurrentCycleAccessingMemory, isCurrentCycleUsingIDU)
            if (
                (op.isAccessingMemory() and isCurrentCycleAccessingMemory) 
                or
                (op.isUsingIDU() and isCurrentCycleUsingIDU)  
            ):
                # entre new cycle
                cycles += 4
                # cleaning the state as we enter to the new cycle
        ...
        # if isCurrentCycleAccessingMemory and isCurrentCycleUsingIDU:
        #     cycles += 4
            # print("new cycle fetch")
        return cycles
    
# pushed the context into SP addr
push_instruction = (
    Instruction("push")
    .add(ExtraCycle())
...
# laod opernad into Z
load_operand_z = (
    Instruction("load operand intp z")
    .add(LoadOperand())
    .add(StoreInRegister("Z"))
)
# load operand into W
...
for dest_key, dest_value in create_indexed_map(0x40, 0x08, R8).items(): 
    for src_key, src_value in create_indexed_map(dest_key, 0x01, R8).items():
        if dest_value == src_value == "(HL)":
            INSTRUCTIONS[src_key] = Instruction("HALT").add(Halt())
        else:
...
for key, value in create_indexed_map(0xC0, 0x08, COND).items():
    INSTRUCTIONS[key] = (
        ...
        # pop
        .addSubInstruction(pop_instruction)
...
for key, value in create_indexed_map(0xC4, 0x08, COND).items():
    INSTRUCTIONS[key] = (
        ...
        # push
        .addSubInstruction(push_instruction)
...
INSTRUCTIONS[0xFB] = (
    Instruction("ei")
    .add(EnableDisableIME(True, with_delay=True))
    )
...
def get_handling_interrupt_instruction(type):
    return (
        Instruction("INTERRUPT {}".format(type))
        .add(ExtraCycle())
        .add(ExtraCycle())
