from base.cpu import Cpu, CyceledCpu
...
class Debuger:
        ...
        match self.mode:
            ...
            case "skip":
                if self.skip_repeat == 0:
                    ...
                    self.cpu_tick(False)
                    if self.target_addr == cpu.registerFile.read("PC"):
                        self.skip_repeat -= 1
    ...
    def cpu_tick(self, vebose=True): 
        pc = self.cpu.registerFile.read("PC")
        ...
        if pc == self.cpu.registerFile.read("PC"):
            ...
            match self.command[0]:
                case "m":
                    try:
                        ...
                        print("Addr {:04X}: {:02X}".format(addr, cpu.memory.read(addr)))
